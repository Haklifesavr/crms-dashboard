/**
 * App Kanban
 */

'use strict';

(async function () {
  console.log("URL IN KANBAN", BASE_URL)
  let boards;
  const kanbanSidebar = document.querySelector('.kanban-update-item-sidebar'),
    kanbanWrapper = document.querySelector('.kanban-wrapper'),
    commentEditor = document.querySelector('.comment-editor'),
    kanbanAddNewBoard = document.querySelector('.kanban-add-new-board'),
    kanbanAddNewInput = [].slice.call(document.querySelectorAll('.kanban-add-board-input')),
    kanbanAddBoardBtn = document.querySelector('.kanban-add-board-btn'),
    datePicker = document.querySelector('#due-date'),
    select2 = $('.select2'), // ! Using jquery vars due to select2 jQuery dependency
    assetsPath = document.querySelector('html').getAttribute('data-assets-path');

  // // Init kanban Offcanvas
  const kanbanOffcanvas = new bootstrap.Offcanvas(kanbanSidebar);
  const kanbanResponse = await fetch(`${BASE_URL}/api/kanban-data/`);
  if (!kanbanResponse.ok) {
    console.error('Error fetching kanban data:', kanbanResponse.statusText);
  } else {
    boards = await kanbanResponse.json();
  }

  // datepicker init
  if (datePicker) {
    datePicker.flatpickr({
      monthSelectorType: 'static',
      altInput: true,
      altFormat: 'j F, Y',
      dateFormat: 'Y-m-d'
    });
  }

  //! TODO: Update Event label and guest code to JS once select removes jQuery dependency
  // select2
  if (select2.length) {
    function renderLabels(option) {
      if (!option.id) {
        return option.text;
      }
      var $badge = "<div class='badge " + $(option.element).data('color') + " rounded-pill'> " + option.text + '</div>';
      return $badge;
    }

    select2.each(function () {
      var $this = $(this);
      $this.wrap("<div class='position-relative'></div>").select2({
        placeholder: 'Select Label',
        dropdownParent: $this.parent(),
        templateResult: renderLabels,
        templateSelection: renderLabels,
        escapeMarkup: function (es) {
          return es;
        }
      });
    });
  }

  // Comment editor
  if (commentEditor) {
    new Quill(commentEditor, {
      modules: {
        toolbar: '.comment-toolbar'
      },
      placeholder: 'Write a Comment... ',
      theme: 'snow'
    });
  }


  // Render board dropdown
  function renderBoardDropdown(boardId) {
    return (
      "<div class='dropdown'>" +
      "<i class='dropdown-toggle ti ti-dots-vertical cursor-pointer' id='board-dropdown' data-bs-toggle='dropdown' aria-haspopup='true' aria-expanded='false'></i>" +
      "<div class='dropdown-menu dropdown-menu-end' aria-labelledby='board-dropdown'>" +
      "<a class='dropdown-item delete-board' href='javascript:void(0)' data-board-id='" + boardId + "'> <i class='ti ti-trash ti-xs' me-1></i> <span class='align-middle'>Delete</span></a>" +
      "<a class='dropdown-item' href='javascript:void(0)'><i class='ti ti-edit ti-xs' me-1></i> <span class='align-middle'>Rename</span></a>" +
      "<a class='dropdown-item' href='javascript:void(0)'><i class='ti ti-archive ti-xs' me-1></i> <span class='align-middle'>Archive</span></a>" +
      '</div>' +
      '</div>'
    );
  }
  // Render item dropdown
  function renderDropdown(taskId) {
    return (
      "<div class='dropdown kanban-tasks-item-dropdown'>" +
      "<i class='dropdown-toggle ti ti-dots-vertical' id='kanban-tasks-item-dropdown' data-bs-toggle='dropdown' aria-haspopup='true' aria-expanded='false'></i>" +
      "<div class='dropdown-menu dropdown-menu-end' aria-labelledby='kanban-tasks-item-dropdown'>" +
      "<a class='dropdown-item' href='javascript:void(0)'>Copy task link</a>" +
      "<a class='dropdown-item' href='javascript:void(0)'>Duplicate task</a>" +
      "<a class='dropdown-item delete-task' href='javascript:void(0)' data-task-id='" + taskId + "'>Delete</a>" +
      '</div>' +
      '</div>'
    );
  }
  // Render header
  function renderHeader(color, text) {
    return (
      "<div class='d-flex justify-content-between flex-wrap align-items-center mb-2 pb-1'>" +
      "<div class='item-badges'> " +
      "<div class='badge rounded-pill bg-label-" +
      color +
      "'> " +
      text +
      '</div>' +
      '</div>' +
      renderDropdown() +
      '</div>'
    );
  }

    function renderAvatar(images, pullUp, size, margin, members) {
      var defaultAvatar = 'img/avatars/1.png'; 
    
      var $transition = pullUp ? ' pull-up' : '',
        $size = size ? 'avatar-' + size + '' : '',
        member = members == undefined ? ' ' : members.split(',');
    
      return (images ? images.split(',') : ['']).map(function (img, index, arr) {
        var $margin = margin && index !== arr.length - 1 ? ' me-' + margin + '' : '';
        var avatarSrc = img ? assetsPath + 'img/avatars/' + img : assetsPath + defaultAvatar;
    
        return (
          "<div class='avatar " +
          $size +
          $margin +
          "'" +
          "data-bs-toggle='tooltip' data-bs-placement='top'" +
          "title='" +
          (member[index] || '') +
          "'" +
          '>' +
          "<img src='" +
          avatarSrc +
          "' alt='Avatar' class='rounded-circle " +
          $transition +
          "'>" +
          '</div>'
        );
      }).join(' ');
  }
    
  // Render footer
  function renderFooter(attachments, comments, assigned, members) {
    return (
      "<div class='d-flex justify-content-between align-items-center flex-wrap mt-2 pt-1'>" +
      "<div class='d-flex'> <span class='d-flex align-items-center me-2'><i class='ti ti-paperclip ti-xs me-1'></i>" +
      "<span class='attachments'>" +
      attachments +
      '</span>' +
      "</span> <span class='d-flex align-items-center ms-1'><i class='ti ti-message-dots ti-xs me-1'></i>" +
      '<span> ' +
      comments +
      ' </span>' +
      '</span></div>' +
      "<div class='avatar-group d-flex align-items-center assigned-avatar'>" +
      renderAvatar(assigned, true, 'xs', null, members) +
      '</div>' +
      '</div>'
    );
  }
  
  const kanban = new jKanban({
    element: '.kanban-wrapper',
    gutter: '15px',
    widthBoard: '250px',
    dragItems: true,
    boards: boards,
    dragBoards: true,
    addItemButton: true,
    buttonContent: '+ Add Item',
    itemAddOptions: {
      enabled: true, // add a button to board for easy item creation
      content: '+ Add New Item', // text or html content of the board button
      class: 'kanban-title-button btn', // default class of the button
      footer: false // position the button on footer
    },

    dragEl: function (el, source) {
      console.log('Item is being dragged from board:', source.parentNode.dataset.id);
      // You can add custom behavior when an item starts to drag
      el.setAttribute('data-source-board-id', source.parentNode.dataset.id);
    },
    // dropEl function within jKanban initialization
    dropEl: async function (el, target, source, sibling) {
      let itemId = el.getAttribute('data-eid'); // Ensure this is the custom string ID, e.g., 'item-1-4'
      let newBoardId = target.parentNode.getAttribute('data-id'); // Ensure this is the custom string ID, e.g., 'board-1-26'
    
      const csrfToken = getCookie('csrftoken');
      console.log("Item ID:", itemId, "New Board ID:", newBoardId);
      
      try {
        const response = await fetch(`${BASE_URL}/api/update-item-board/${itemId}/`, {
          method: 'PUT',
          headers: {
            'X-CSRFToken': csrfToken,
            'Content-Type': 'application/json'
          },
          body: JSON.stringify({ boardId: newBoardId })
        });
    
        if (!response.ok) {
          throw new Error('Failed to update item board ID');
        }
    
        console.log('Item successfully moved to the new board:', newBoardId);
      } catch (error) {
        console.error('Error updating item board ID:', error);
      }
    },

    dragendEl: function (el) {
      console.log('Item drag ended:', el.dataset.eid);
      // Optional: Reset any visual changes or temporary state
    },
    
    
    click: function (el) {
      let element = el;
      let taskId = element.getAttribute('data-eid');
      let title = element.getAttribute('data-eid')
          ? element.querySelector('.kanban-text').textContent
          : element.textContent,
        date = element.getAttribute('data-due-date'),
        dateObj = new Date(),
        year = dateObj.getFullYear(),
        dateToUse = date
          ? date + ', ' + year
          : dateObj.getDate() + ' ' + dateObj.toLocaleString('en', { month: 'long' }) + ', ' + year,
        label = element.getAttribute('data-badge-text'),
        avatars = element.getAttribute('data-assigned');


      // Show kanban offcanvas
      kanbanOffcanvas.show();

      // To get data on sidebar
      kanbanSidebar.querySelector('#title').value = title;
      kanbanSidebar.querySelector('#due-date').nextSibling.value = dateToUse;

      // ! Using jQuery method to get sidebar due to select2 dependency
      $('.kanban-update-item-sidebar').find(select2).val(label).trigger('change');

      // Remove & Update assigned
      kanbanSidebar.querySelector('.assigned').innerHTML = '';
      kanbanSidebar
        .querySelector('.assigned')
        .insertAdjacentHTML(
          'afterbegin',
          renderAvatar(avatars, false, 'xs', '1', el.getAttribute('data-members')) +
            "<div class='avatar avatar-xs ms-1'>" +
            "<span class='avatar-initial rounded-circle bg-label-secondary'><i class='ti ti-plus ti-xs text-heading'></i></span>" +
            '</div>'
        );
        // Add event listener for form submission
        kanbanSidebar.querySelector('form').addEventListener('submit', async function (event) {
          event.preventDefault(); 

          // Get updated task details from form
          let updatedTitle = kanbanSidebar.querySelector('#title').value;
          let updatedDueDate = kanbanSidebar.querySelector('#due-date').nextSibling.value;
          // let updatedLabel = $('.kanban-update-item-sidebar').find('.select2').val();
          
          // Construct the data object to be sent in the request body
          let csrfToken = getCookie('csrftoken');
          let formData = new FormData(event.target);
          formData.append('title', updatedTitle);
          formData.append('due_date', updatedDueDate);

          // Send PUT request to update the task
          try {
            const response = await fetch(`${BASE_URL}/api/update-item/${taskId}/`, {
              method: 'PUT',
              headers: {
                'X-CSRFToken': csrfToken
              },
              body: formData
            });

            if (response.ok) {
              // Task updated successfully, you may want to handle this event appropriately
              console.log('Task updated successfully');
              // Optionally, you can reload the kanban board or update the UI to reflect the changes
            } else {
              console.error('Error updating task:', response.statusText);
              // Handle errors appropriately, like showing an error message to the user
            }
          } catch (error) {
            console.error('Error updating task:', error.message);
            // Handle network errors or other exceptions
          }
        });
  },

    buttonClick: function (el, boardId) {
      const addNew = document.createElement('form');
      addNew.setAttribute('class', 'new-item-form');
      addNew.innerHTML = `
        <div class="mb-3">
          <textarea name="title" class="form-control add-new-item" rows="2" placeholder="Add Content" required></textarea>
        </div>
        <div class="mb-3">
          <button type="submit" class="btn btn-primary btn-sm me-2 waves-effect waves-light">Add</button>
          <button type="button" class="btn btn-label-secondary btn-sm cancel-add-item waves-effect waves-light">Cancel</button>
        </div>`;
      kanban.addForm(boardId, addNew);
    
      addNew.addEventListener('submit', async function (e) {
        e.preventDefault();
    
        const title = e.target.querySelector('.add-new-item').value;
        const csrfToken = getCookie('csrftoken'); // Get CSRF token for security
    
        const formData = new FormData();
        formData.append('title', title);
        console.log("boardid in create item", boardId)
        formData.append('board_id', boardId);
    
        try {
          const response = await fetch(`${BASE_URL}/api/create-item/`, {
            method: 'POST',
            headers: {
              'X-CSRFToken': csrfToken
            },
            body: formData
          });
    
          if (response.ok) {
            const newTaskData = await response.json(); // Parse response data
    
            kanban.addElement(boardId, {
              title: "<span class='kanban-text'>" + newTaskData.title + '</span>',
              id: newTaskData.id
            });
    
            // Add dropdown in new boards
            const kanbanText = [].slice.call(document.querySelectorAll('.kanban-board[data-id="' + boardId + '"] .kanban-text'));
            kanbanText.forEach(function (e) {
              e.insertAdjacentHTML('beforebegin', renderDropdown());
            });
    
            // Prevent sidebar from opening on click of dropdown buttons of new tasks
            const newTaskDropdown = [].slice.call(document.querySelectorAll('.kanban-item .kanban-tasks-item-dropdown'));
            if (newTaskDropdown) {
              newTaskDropdown.forEach(function (e) {
                e.addEventListener('click', function (el) {
                  el.stopPropagation();
                });
              });
            }
    
            // Delete tasks for new boards
            const deleteTask = [].slice.call(document.querySelectorAll('.kanban-board[data-id="' + boardId + '"] .delete-task'));
            deleteTask.forEach(function (e) {
              e.addEventListener('click', function () {
                const id = this.closest('.kanban-item').getAttribute('data-eid');
                kanban.removeElement(id);
              });
            });
    
            // Clear the form
            e.target.querySelector('.add-new-item').value = '';
    
            // Remove the form from the DOM
            addNew.remove();
          } else {
            console.error('Error creating task:', response.statusText);
            // Handle errors appropriately, like showing an error message to the user
          }
        } catch (error) {
          console.error('Error:', error);
        }
      });
    
      // Remove form on clicking cancel button
      addNew.querySelector('.cancel-add-item').addEventListener('click', function (e) {
        addNew.remove();
      });
    }    
  });


  // Kanban Wrapper scrollbar
  if (kanbanWrapper) {
    new PerfectScrollbar(kanbanWrapper);
  }

  const kanbanContainer = document.querySelector('.kanban-container'),
    kanbanTitleBoard = [].slice.call(document.querySelectorAll('.kanban-title-board')),
    kanbanItem = [].slice.call(document.querySelectorAll('.kanban-item'));

  // Render custom items
  if (kanbanItem) {
    kanbanItem.forEach(function (el) {
      const element = "<span class='kanban-text'>" + el.textContent + '</span>';
      let img = '';
      if (el.getAttribute('data-image') !== null) {
        img =
          "<img class='img-fluid rounded mb-2' src='" +
          assetsPath +
          'img/elements/' +
          el.getAttribute('data-image') +
          "'>";
      }
      el.textContent = '';
      if (el.getAttribute('data-badge') !== undefined && el.getAttribute('data-badge-text') !== undefined) {
        el.insertAdjacentHTML(
          'afterbegin',
          renderHeader(el.getAttribute('data-badge'), el.getAttribute('data-badge-text')) + img + element
        );
      }
      if (
        el.getAttribute('data-comments') !== undefined ||
        el.getAttribute('data-due-date') !== undefined ||
        el.getAttribute('data-assigned') !== undefined
      ) {
        el.insertAdjacentHTML(
          'beforeend',
          renderFooter(
            el.getAttribute('data-attachments'),
            el.getAttribute('data-comments'),
            el.getAttribute('data-assigned'),
            el.getAttribute('data-members')
          )
        );
      }
    });
  }

  // To initialize tooltips for rendered items
  const tooltipTriggerList = [].slice.call(document.querySelectorAll('[data-bs-toggle="tooltip"]'));
  tooltipTriggerList.map(function (tooltipTriggerEl) {
    return new bootstrap.Tooltip(tooltipTriggerEl);
  });

  // prevent sidebar to open onclick dropdown buttons of tasks
  const tasksItemDropdown = [].slice.call(document.querySelectorAll('.kanban-tasks-item-dropdown'));
  if (tasksItemDropdown) {
    tasksItemDropdown.forEach(function (e) {
      e.addEventListener('click', function (el) {
        el.stopPropagation();
      });
    });
  }

  // Toggle add new input and actions add-new-btn
  if (kanbanAddBoardBtn) {
    kanbanAddBoardBtn.addEventListener('click', () => {
      kanbanAddNewInput.forEach(el => {
        el.value = '';
        el.classList.toggle('d-none');
      });
    });
  }

  // Render add new inline with boards
  if (kanbanContainer) {
    kanbanContainer.appendChild(kanbanAddNewBoard);
  }

  // Makes kanban title editable for rendered boards
  if (kanbanTitleBoard) {
    kanbanTitleBoard.forEach(function (elem) {
      elem.addEventListener('mouseenter', function () {
        this.contentEditable = 'true';
      });

      // Appends delete icon with title
      elem.insertAdjacentHTML('afterend', renderBoardDropdown());
    });
  }


  document.addEventListener('click', function(e) {
    if (e.target && e.target.matches('.delete-board')) {
      e.preventDefault(); // Prevent default action if it's a link or has some default action.
      
      // Retrieve the board ID from the clicked board element
      const boardId = e.target.closest('.kanban-board').getAttribute('data-id');
  
      // Call deleteBoard function with the retrieved board ID
      deleteBoard(boardId).then(() => {
        kanban.removeBoard(boardId); // Only remove the board from the UI if the backend deletion was successful.
      }).catch((error) => {
        console.error("Failed to delete the board:", error);
        // Optionally, show an error message to the user.
      });
    }
  });
  

   // Function to delete a board
   async function deleteBoard(boardId) {
    const csrfToken = getCookie('csrftoken');
    try {
      const response = await fetch(`${BASE_URL}/api/delete-board/${boardId}/`, {
        method: 'DELETE',
        headers: {
          'X-CSRFToken': csrfToken,
          'Content-Type': 'application/json'
        }
      });

      if (!response.ok) {
        throw new Error('Failed to delete board');
      }

      console.log('Board deleted successfully');
      // Remove the board from UI
      kanban.removeBoard(boardId);
    } catch (error) {
      console.error('Error deleting board:', error);
    }
  }
  
  // Delete task for rendered boards
  const deleteTask = [].slice.call(document.querySelectorAll('.delete-task'));
  if (deleteTask) {
    deleteTask.forEach(function (e) {
      e.addEventListener('click', function () {
        // Retrieve the task ID from the clicked task element
        const itemId = this.closest('.kanban-item').getAttribute('data-eid');

        // Call deleteItem function with the retrieved task ID
        deleteItem(itemId).then(() => {
          console.log(`Task with ID ${itemId} deleted successfully.`);
          kanban.removeElement(itemId); // Remove the task from the UI if the backend deletion was successful.
        }).catch((error) => {
          console.error("Failed to delete the task:", error);
          // Optionally, show an error message to the user.
        });
      });
    });
  }

  async function deleteItem(itemId) {
      const csrfToken = getCookie('csrftoken');
      try {
          const response = await fetch(`${BASE_URL}/api/delete-item/${itemId}/`, {
              method: 'DELETE',
              headers: {
                  'X-CSRFToken': csrfToken
              }
          });

          if (!response.ok) {
              throw new Error('Failed to delete item');
          }

          console.log('Item deleted successfully');
          // Optionally, remove the item from the UI here
      } catch (error) {
          console.error('Error deleting item:', error);
      }
  }

  // Cancel btn add new input
  const cancelAddNew = document.querySelector('.kanban-add-board-cancel-btn');
  if (cancelAddNew) {
    cancelAddNew.addEventListener('click', function () {
      kanbanAddNewInput.forEach(el => {
        el.classList.toggle('d-none');
      });
    });
  }

  
  // ----------- Add new board ------------ //
  if (kanbanAddNewBoard) {
    kanbanAddNewBoard.addEventListener('submit', async function (e) {
      e.preventDefault();
      const thisEle = this,
        value = thisEle.querySelector('.form-control').value;

      const csrfToken = getCookie('csrftoken');
      // API call to create a new board
      const response = await fetch(`${BASE_URL}/api/create-board/`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'X-CSRFToken': csrfToken
        },
        body: JSON.stringify({ title: value })
      });

      if (response.ok) {
        const { id, title } = await response.json();

        // Create a new board based on the API response
        kanban.addBoards([
          {
            id: id, // Ensure this 'id' is unique and correctly formatted
            title: title,
            dragTo: [], // Consider dynamically setting this based on existing boards
          }
        ]);

        // Logic below is to add UI functionalities like edit and delete to the new board
        const kanbanBoardLastChild = document.querySelectorAll('.kanban-board:last-child')[0];
        if (kanbanBoardLastChild) {
          const header = kanbanBoardLastChild.querySelector('.kanban-title-board');
          header.insertAdjacentHTML('afterend', renderBoardDropdown());

          kanbanBoardLastChild.querySelector('.kanban-title-board').addEventListener('mouseenter', function () {
            this.contentEditable = 'true';
          });

          const deleteNewBoards = kanbanBoardLastChild.querySelector('.delete-board');
          if (deleteNewBoards) {
            deleteNewBoards.addEventListener('click', function () {
              const id = this.closest('.kanban-board').getAttribute('data-id');
              // Consider an API call here to also delete the board from the backend
              kanban.removeBoard(id);
            });
          }
        }

        // Reset the add new board form to be ready for another input
        kanbanAddNewInput.forEach(el => {
          el.classList.add('d-none');
        });

        // Ensure the add new board button/form is correctly positioned
        if (kanbanContainer) {
          kanbanContainer.appendChild(kanbanAddNewBoard);
        }
      } else {
        // Log or handle error response from API
        console.error('Failed to create board');
      }
    });
  }

    
  // Function to retrieve CSRF token from cookies
  function getCookie(name) {
    const value = `; ${document.cookie}`;
    const parts = value.split(`; ${name}=`);
    if (parts.length === 2) return parts.pop().split(';').shift();
  }
  
  
  // Clear comment editor on close
  kanbanSidebar.addEventListener('hidden.bs.offcanvas', function () {
    kanbanSidebar.querySelector('.ql-editor').firstElementChild.innerHTML = '';
  });

  // Re-init tooltip when offcanvas opens(Bootstrap bug)
  if (kanbanSidebar) {
    kanbanSidebar.addEventListener('shown.bs.offcanvas', function () {
      const tooltipTriggerList = [].slice.call(kanbanSidebar.querySelectorAll('[data-bs-toggle="tooltip"]'));
      tooltipTriggerList.map(function (tooltipTriggerEl) {
        return new bootstrap.Tooltip(tooltipTriggerEl);
      });
    });
  }
})();